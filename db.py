...
def truncate_rekap_penggunaan_ruangan():
    with app.app_context():
        ...
        try:
            ...
            mysql.connection.commit()
            print("Table kelas_prodi has been truncated.")
        except Exception as e:
            mysql.connection.rollback()
...
def truncate_heatmap():
    with app.app_context():
        cur = mysql.connection.cursor()
        try:
            cur.execute("TRUNCATE TABLE heatmap")
            mysql.connection.commit()
            print("Table heatmap has been truncated.")
        except Exception as e:
            mysql.connection.rollback()
            logging.error(f"Error truncating table heatmap: {e}")
        finally:
            cur.close()

def truncate_kapasitas():
    with app.app_context():
        cur = mysql.connection.cursor()
        try:
            cur.execute("TRUNCATE TABLE kapasitas_ruangan")
            mysql.connection.commit()
            print("Table kapasitas_ruangan has been truncated.")
        except Exception as e:
            mysql.connection.rollback()
            logging.error(f"Error truncating table kapasitas Ruangan: {e}")
        finally:
            cur.close()
