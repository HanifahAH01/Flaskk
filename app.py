...
from db import insert_data_kapasitas_ruangan, insert_data_kapasitas,insert_table_heatmap,truncate_kapasitas,truncate_heatmap,truncate_jadwal_ruangan, insert_real_data_jadwal,truncate_rekap_penggunaan_ruangan,insert_program_studi
from heatmap import generate_plot
import mysql.connector
from kapasitas import Kapasitas
from jadwal1 import Jadwal_json
...
@app.route('/run-heatmap', methods=['POST'])
def run_heatmap():
    try:
...
def run_data_jadwal():
    try:
        scraper2.get_data_jadwal()
        scraper3.get_data_jadwal()
        insert_real_data_jadwal()
        return jsonify({'message': 'Data Jadwal Berhasil Di-generate'})
    except Exception as e:
        return jsonify({'error': str(e)})
    
@app.route('/run-data-kapasitas', methods=['POST'])
def run_data_kapasitas():
    try:
...
@app.route('/run-empty-heatmap', methods=['POST'])
def run_empty_heatmap():
    try:
        truncate_heatmap()
        return jsonify({'message': 'Data Jadwal Berhasil Di Hapus'})
    except Exception as e:
...
@app.route('/run-empty-kapasitas', methods=['POST'])
def run_empty_kapasitas():
    try:
        truncate_kapasitas()
        return jsonify({'message': 'Data Jadwal Berhasil Di Hapus'})
    except Exception as e:
